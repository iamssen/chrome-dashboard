...
async function listTask(): Promise<Task[]> {
  const res: Response = await fetch('/tasks/list?onlyCompleted=false&excludeWontFix=true&type=tasks-assigned-to-me&padId=');
...
async function padList(): Promise<PadListDoc[]> {
  ...
  const body: string = encodeURI(`query=${PAD_LIST}&variables=${JSON.stringify({
    filter: 1,
    limit: 1000,
  })}&xsrf=${xsrf}`);
...
async function padListFolder(folderId: string, isFavorite: boolean): Promise<PadListFolder> {
  const xsrf: string = get('xsrf') || '';
  const body: string = encodeURI(`query=${PAD_LIST_FOLDER}&variables=${JSON.stringify({encryptedFolderId: folderId})}&xsrf=${xsrf}`);
...
export async function getDropboxPaper(): Promise<DropboxPaperData> {
  ...
  const folders: PadListFolder[] = await Promise.all<PadListFolder>(folderList.map(({folder}) => padListFolder(folder.encryptedId, favorites.has(folder.encryptedId))));
